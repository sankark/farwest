...
/**
	...
	Search for "topac" to find modifications.
*/
...
draw2d.EventTarget.prototype.dispatchEvent=function(_60e8){
	...
	if(typeof this.eventhandlers[_60e8.type]!="undefined"){
		for(var i=0;
		i<this.eventhandlers[_60e8.type].length;
		i++){
...
draw2d.EventTarget.prototype.removeEventListener=function(sType,_60eb){
	if(typeof this.eventhandlers[sType]!="undefined"){
		var _60ec=new Array;
		for(var i=0;
		i<this.eventhandlers[sType].length;
		i++){
...
draw2d.UUID.create=function(){
	...
	return (_5f17()+_5f17()+"-"+_5f17()+"-"+_5f17()+"-"+_5f17()+"-"+_5f17()+_5f17()+_5f17());
	}
;
...
draw2d.ArrayList.prototype.getPersistentAttributes=function(){
	return {
		data:this.data,increment:this.increment,size:this.getSize()}	
	;
...
function openwindow(url,width,_5f1c){
	...
	property="left="+left+", top="+top+", toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,alwaysRaised,width="+width+",height="+_5f1c;
...
draw2d.Draggable.prototype.attachEventHandlers=function(){
	...
	this.tempMouseMove=function(){
		...
		var _4bd8=new draw2d.Point(_4bd7.clientX-oThis.diffX,_4bd7.clientY-oThis.diffY);
		...
		var _4bdd=oThis.getDropTarget(_4bd7.clientX+_4bd9-_4bdb,_4bd7.clientY+_4bda-_4bdc);
		var _4bde=oThis.getCompartment(_4bd7.clientX+_4bd9-_4bdb,_4bd7.clientY+_4bda-_4bdc);
		if(draw2d.Drag.currentTarget!=null&&_4bdd!=draw2d.Drag.currentTarget){
		...
		if(draw2d.Drag.currentCompartment!=null&&_4bde!=draw2d.Drag.currentCompartment){
			var _4bdf=new draw2d.DragDropEvent();
			_4bdf.initDragDropEvent("figureleave",false,oThis);
			draw2d.Drag.currentCompartment.dispatchEvent(_4bdf);
			}		
		if(_4bde!=null&&_4bde.node!=oThis.node&&_4bde!=draw2d.Drag.currentCompartment){
			var _4bdf=new draw2d.DragDropEvent();
	...
	oThis.tempMouseUp=function(){
		...
		var _4be6=oThis.getDropTarget(_4be1.clientX+_4be2-_4be4,_4be1.clientY+_4be3-_4be5);
		var _4be7=oThis.getCompartment(_4be1.clientX+_4be2-_4be4,_4be1.clientY+_4be3-_4be5);
		if(_4be6!=null){
...
draw2d.Draggable.prototype.getCompartment=function(x,y){
	var _4bf0=null;
	for(var i=0;
	i<this.node.workflow.compartments.getSize();
	i++){
...
draw2d.Color.prototype.getIdealTextColor=function(){
	...
	return (255-_4f14<_4f13)?new draw2d.Color(0,0,0):new draw2d.Color(255,255,255);
	}
;
...
draw2d.Color.prototype.hex2rgb=function(_4f15){
	_4f15=_4f15.replace("#","");
	return ({
		0:parseInt(_4f15.substr(0,2),16),1:parseInt(_4f15.substr(2,2),16),2:parseInt(_4f15.substr(4,2),16)}	
	);
	}
;
...
draw2d.SnapToGrid.prototype.snapPoint=function(_51dd,_51de,_51df){
	_51df.x=this.workflow.gridWidthX*Math.floor(((_51de.x+this.workflow.gridWidthX/2)/this.workflow.gridWidthX));
	_51df.y=this.workflow.gridWidthY*Math.floor(((_51de.y+this.workflow.gridWidthY/2)/this.workflow.gridWidthY));
	return 0;
	}
;
...
draw2d.SnapToGeometry.prototype.snapRectangle=function(_4eca,_4ecb){
	...
	var _4ece=this.snapPoint(draw2d.SnapToHelper.NORTH_WEST,_4eca.getTopLeft(),_4ecc);
	_4ecb.x=_4ecc.x;
	_4ecb.y=_4ecc.y;
	var _4ecf=this.snapPoint(draw2d.SnapToHelper.SOUTH_EAST,_4eca.getBottomRight(),_4ecd);
...
draw2d.SnapToGeometry.prototype.populateRowsAndCols=function(){
	...
	i++){
		...
		if(_4ed3!=this.workflow.getCurrentSelection()){
			...
			this.cols[index*3]=new draw2d.SnapToGeometryEntry(-1,_4ed4.getX());
			this.rows[index*3]=new draw2d.SnapToGeometryEntry(-1,_4ed4.getY());
			this.cols[index*3+1]=new draw2d.SnapToGeometryEntry(0,_4ed4.x+(_4ed4.getWidth()-1)/2);
			this.rows[index*3+1]=new draw2d.SnapToGeometryEntry(0,_4ed4.y+(_4ed4.getHeight()-1)/2);
			this.cols[index*3+2]=new draw2d.SnapToGeometryEntry(1,_4ed4.getRight()-1);
			this.rows[index*3+2]=new draw2d.SnapToGeometryEntry(1,_4ed4.getBottom()-1);
			index++;
			}		
		}	
	}
...
draw2d.Figure.prototype.createCommand=function(_5afd){
	...
	if(_5afd.getPolicy()==draw2d.EditPolicy.DELETE){
		...
		/**
		@author topac
		@original return new draw2d.CommandDelete(this);
		*/
		if ( this.getModel() instanceof State)
			return new CommandDeleteState(this);
		else if ( this.getModel() instanceof Page)
			return new CommandDeletePage(this);
		else
...
draw2d.Port.prototype.setUiRepresentation=function(_519f){
	//alert('draw2d.Port.prototype.setUiRepresentation');
	if(_519f==null){
	...
	//alert('draw2d.Port.prototype.setUiRepresentation(END)');
}
;
...
draw2d.Port.prototype.detachMoveListener=function(_51ad){
	//alert('draw2d.Port.prototype.detachMoveListener');
...
draw2d.Port.prototype.createCommand=function(_51c0){
	...
	if(_51c0.getPolicy()==draw2d.EditPolicy.CONNECT){
		if(_51c0.source.parentNode.id==_51c0.target.parentNode.id){
			...
			return new draw2d.CommandConnect(_51c0.canvas,_51c0.source,_51c0.target);
			}		
		}	
	return null;
	}
...
draw2d.InputPort.prototype.createCommand=function(_528a){
	if(_528a.getPolicy()==draw2d.EditPolicy.CONNECT){
		...
		if(_528a.source instanceof draw2d.OutputPort){
			return new draw2d.CommandConnect(_528a.canvas,_528a.source,_528a.target);
			}		
		return null;
		}	
...
draw2d.OutputPort.prototype.createCommand=function(_48da){
	if(_48da.getPolicy()==draw2d.EditPolicy.CONNECT){
		...
		if(_48da.source instanceof draw2d.InputPort){
			return new draw2d.CommandConnect(_48da.canvas,_48da.target,_48da.source);
			}		
		return null;
		}	
...
draw2d.ManhattanConnectionRouter.prototype._route=function(conn,_5a5b,_5a5c,toPt,toDir){
	var TOL=0.1;
	var _5a60=0.01;
	var UP=0;
	var RIGHT=1;
		...
		if(_5a5c==RIGHT){
			...
			if(_5a5c==DOWN){
				if(((xDiff*xDiff)<TOL)&&(yDiff<0)&&(toDir==UP)){
					...
					if(yDiff>0){
						point=new draw2d.Point(_5a5b.x,_5a5b.y+this.MINDIST);
						}					
					else{
						if(((xDiff>0)&&(toDir==RIGHT))||((xDiff<0)&&(toDir==LEFT))){
							...
							if(_5a5c==toDir){
								var pos=Math.max(_5a5b.y,toPt.y)+this.MINDIST;
								point=new draw2d.Point(_5a5b.x,pos);
								}							
							else{
								point=new draw2d.Point(_5a5b.x,_5a5b.y-(yDiff/2));
								}							
							}						
						}					
					if(xDiff>0){
					...
					if(((xDiff*xDiff)<TOL)&&(yDiff>0)&&(toDir==DOWN)){
						...
						if(yDiff<0){
							...
							if(((xDiff>0)&&(toDir==RIGHT))||((xDiff<0)&&(toDir==LEFT))){
								...
								if(_5a5c==toDir){
									var pos=Math.min(_5a5b.y,toPt.y)-this.MINDIST;
									point=new draw2d.Point(_5a5b.x,pos);
									}								
								else{
									point=new draw2d.Point(_5a5b.x,_5a5b.y-(yDiff/2));
...
draw2d.BezierConnectionRouter.prototype.drawBezier=function(_5a8d,_5a8e,t,iter){
...
draw2d.BezierConnectionRouter.prototype.route=function(conn){
	...
	var toPt=conn.getEndPoint();
	this._route(_5a99,conn,toPt,this.getEndDirection(conn),_5a9a,this.getStartDirection(conn));
	var _5a9c=new Array();
...
draw2d.BezierConnectionRouter.prototype._route=function(_5a9e,conn,_5aa0,_5aa1,toPt,toDir){
	var TOL=0.1;
	var _5aa5=0.01;
	var _5aa6=90;
	var UP=0;
					...
					if(yDiff>0){
						...
						if(((xDiff>0)&&(toDir==RIGHT))||((xDiff<0)&&(toDir==LEFT))){
							...
							if(_5aa1==toDir){
								...
								point=new draw2d.Point(_5aa0.x,_5aa0.y-(yDiff/2));
								}							
							}						
						}					
					if(xDiff>0){
...
draw2d.FanConnectionRouter.prototype.route=function(conn){
	...
	i++){
		...
		if(_5bb9.getTarget()==conn.getTarget()||_5bb9.getSource()==conn.getTarget()){
			_5bb6.add(_5bb9);
			if(conn==_5bb9){
				index=_5bb6.getSize();
				}			
...
draw2d.FanConnectionRouter.prototype.routeCollision=function(conn,index){
	...
	var ray;
	if(_5bc1==draw2d.PositionConstants.SOUTH||_5bc1==draw2d.PositionConstants.EAST){
	...
	var _5bc6;
	if(index%2==0){
		_5bc6=new draw2d.Point(_5bc0.x+(index/2)*(-1*_5bc5),_5bc0.y+(index/2)*_5bc4);
		}	
	else{
		_5bc6=new draw2d.Point(_5bc0.x+(index/2)*_5bc5,_5bc0.y+(index/2)*(-1*_5bc4));
		}	
	conn.addPoint(_5bc6);
	conn.addPoint(end);
	}
...
draw2d.Graphics.prototype.fillPolygon=function(_4b70,_4b71){
	...
	i<_4b70.length;
	i++){
		rotX[i]=this.xt+_4b70[i]*this.cosRadian-_4b71[i]*this.sinRadian;
		rotY[i]=this.yt+_4b70[i]*this.sinRadian+_4b71[i]*this.cosRadian;
		}	
...
draw2d.Graphics.prototype.drawPolygon=function(_4b76,_4b77){
	...
	i<_4b76.length;
	i++){
		rotX[i]=this.xt+_4b76[i]*this.cosRadian-_4b77[i]*this.sinRadian;
		rotY[i]=this.yt+_4b76[i]*this.sinRadian+_4b77[i]*this.cosRadian;
		}	
...
draw2d.Connection.prototype.getStartAngle=function(){
	...
	var _5636=Math.sqrt((p1.x-p2.x)*(p1.x-p2.x)+(p1.y-p2.y)*(p1.y-p2.y));
...
draw2d.Connection.prototype.getEndAngle=function(){
	...
	var _563a=Math.sqrt((p1.x-p2.x)*(p1.x-p2.x)+(p1.y-p2.y)*(p1.y-p2.y));
...
draw2d.CanvasDocument.prototype.getFigures=function(){
	...
	i++){
		...
		if(_4f22.indexOf(_4f24)==-1&&_4f24.getParent()==null&&!(_4f24 instanceof draw2d.WindowFigure)){
			_4f20.add(_4f24);
			}		
		}	
	return _4f20;
...
draw2d.ResizeHandle=function(_4e8d,type){
	...
	var _4e90=_4e8f/2;
	switch(this.type){
		case 1:this.setSnapToGridAnchor(new draw2d.Point(_4e8f,_4e8f));
		break;
		case 2:this.setSnapToGridAnchor(new draw2d.Point(_4e90,_4e8f));
		break;
		case 3:this.setSnapToGridAnchor(new draw2d.Point(0,_4e8f));
		break;
		case 4:this.setSnapToGridAnchor(new draw2d.Point(0,_4e90));
		break;
		case 5:this.setSnapToGridAnchor(new draw2d.Point(0,0));
		break;
		case 6:this.setSnapToGridAnchor(new draw2d.Point(_4e90,0));
		break;
		case 7:this.setSnapToGridAnchor(new draw2d.Point(_4e8f,0));
		break;
		case 8:this.setSnapToGridAnchor(new draw2d.Point(_4e8f,_4e90));
		case 9:this.setSnapToGridAnchor(new draw2d.Point(_4e90,_4e90));
		break;
		}	
...
draw2d.ResizeHandle.prototype.getSnapToDirection=function(){
	switch(this.type){
		...
		case 9:return draw2d.SnapToHelper.CENTER;
		}	
	}
;
...
draw2d.ResizeHandle.prototype.onDragstart=function(x,y){
	...
	this.commandMove=_4e96.createCommand(new draw2d.EditPolicy(draw2d.EditPolicy.MOVE));
	this.commandResize=_4e96.createCommand(new draw2d.EditPolicy(draw2d.EditPolicy.RESIZE));
	return true;
	}
;
...
draw2d.ResizeHandle.prototype.onDrag=function(){
	...
	switch(this.type){
		case 1:this.workflow.currentSelection.setPosition(_4e9b-diffX,_4e9c-diffY);
		this.workflow.currentSelection.setDimension(_4e9d+diffX,_4e9e+diffY);
		break;
		case 2:this.workflow.currentSelection.setPosition(_4e9b,_4e9c-diffY);
		this.workflow.currentSelection.setDimension(_4e9d,_4e9e+diffY);
		break;
		case 3:this.workflow.currentSelection.setPosition(_4e9b,_4e9c-diffY);
		this.workflow.currentSelection.setDimension(_4e9d-diffX,_4e9e+diffY);
		break;
		case 4:this.workflow.currentSelection.setPosition(_4e9b,_4e9c);
		this.workflow.currentSelection.setDimension(_4e9d-diffX,_4e9e);
		break;
		case 5:this.workflow.currentSelection.setPosition(_4e9b,_4e9c);
		this.workflow.currentSelection.setDimension(_4e9d-diffX,_4e9e-diffY);
		break;
		case 6:this.workflow.currentSelection.setPosition(_4e9b,_4e9c);
		this.workflow.currentSelection.setDimension(_4e9d,_4e9e-diffY);
		break;
		case 7:this.workflow.currentSelection.setPosition(_4e9b-diffX,_4e9c);
		this.workflow.currentSelection.setDimension(_4e9d+diffX,_4e9e-diffY);
		break;
		case 8:this.workflow.currentSelection.setPosition(_4e9b-diffX,_4e9c);
		this.workflow.currentSelection.setDimension(_4e9d+diffX,_4e9e);
		break;
		}	
...
draw2d.ResizeHandle.prototype.setCanDrag=function(flag){
	...
	switch(this.type){
		...
		break;
		case 9:this.html.style.cursor="resize";
		break;
		}	
	}
;
...
draw2d.Canvas.prototype.addFigure=function(_5a3f,xPos,yPos,_5a42){
	...
	if(_5a3f instanceof draw2d.Line){
		...
		if(obj==null){
			...
			this.html.insertBefore(_5a3f.getHTMLElement(),obj.getHTMLElement());
			}		
		}	
	if(!_5a42){
		_5a3f.paint();
...
draw2d.Canvas.prototype.removeFigure=function(_5a46){
	if(this.enableSmoothFigureHandling==true){
		...
		//alert('this.html.removeChild(_5a46.html)');
		this.html.removeChild(_5a46.html);
		_5a46.setCanvas(null);
		//alert('this.html.removeChild(_5a46.html)(END)');
	}	
	
	if(_5a46 instanceof draw2d.Line){
		//alert('this.canvasLines.remove(_5a46);');
		this.canvasLines.remove(_5a46);
		//alert('this.canvasLines.remove(_5a46);(END)');
...
draw2d.Canvas.prototype.setBackgroundImage=function(_5a4b,_5a4c){
	if(_5a4b!=null){
		if(_5a4c){
			...
			this.html.style.background="transparent url("+_5a4b+") no-repeat";
			}		
		}	
	else{
...
draw2d.Workflow=function(id){
	...
	if(this.html!=null){
		...
		var _4f2b=function(){
			...
			if(oThis.getBestFigure(diffX+_4f2f-_4f31,diffY+_4f30-_4f32)!=null){
				return;
				}			
			var line=oThis.getBestLine(diffX+_4f2f-_4f31,diffY+_4f30-_4f32,null);
			if(line!=null){
				line.onContextMenu(diffX+_4f2f-_4f31,diffY+_4f30-_4f32);
				}			
			else{
				oThis.onContextMenu(diffX+_4f2f-_4f31,diffY+_4f30-_4f32);
				}			
			}		
		;
		...
		var _4f52=function(_4f53){
			...
			var line=oThis.getBestLine(diffX+_4f56-_4f58,diffY+_4f57-_4f59,null);
			if(line!=null){
				line.onDoubleClick();
				}			
			}		
		...
		if(this.html.addEventListener){
			...
			this.html.addEventListener("keydown",_4f35,false);
			...
			if(this.html.attachEvent){
				...
				this.html.attachEvent("onkeydown",_4f35);
...
draw2d.Workflow.prototype.removeFigure=function(_4f89){
	...
	//alert('workflow:removefigure '+ (_4f89.getModel) ? _4f89.getModel() : _4f89);
...
draw2d.Workflow.prototype.getBestCompartmentFigure=function(x,y,_4f8e){
	...
	i++){
		...
		if((_4f91 instanceof draw2d.CompartmentFigure)&&_4f91.isOver(x,y)==true&&_4f91!=_4f8e){
			if(_4f8f==null){
				_4f8f=_4f91;
				}			
			else{
...
draw2d.Workflow.prototype.setCurrentSelection=function(_4fa4){
	...
	for(var i=0;
	i<this.selectionListeners.getSize();
	i++){
...
draw2d.Workflow.prototype.unregisterPort=function(port){
	...
	//alert('workflow:unserigisterport')
...
draw2d.Workflow.prototype.showLineResizeHandles=function(_4fb1){
	...
	draw2d.Canvas.prototype.addFigure.call(this,this.resizeHandleStart,_4fb4.x-_4fb2,_4fb4.y-_4fb2);
	draw2d.Canvas.prototype.addFigure.call(this,this.resizeHandleEnd,_4fb5.x-_4fb2,_4fb5.y-_4fb2);
...
draw2d.Workflow.prototype.showResizeHandles=function(_4fb6){
	...
	var xPos=_4fb6.getX();
	var yPos=_4fb6.getY();
	draw2d.Canvas.prototype.addFigure.call(this,this.resizeHandle1,xPos-_4fb7,yPos-_4fb8);
	draw2d.Canvas.prototype.addFigure.call(this,this.resizeHandle3,xPos+_4fba,yPos-_4fb8);
	draw2d.Canvas.prototype.addFigure.call(this,this.resizeHandle5,xPos+_4fba,yPos+_4fb9);
	draw2d.Canvas.prototype.addFigure.call(this,this.resizeHandle7,xPos-_4fb7,yPos+_4fb9);
	...
	if(_4fb6.isStrechable()&&_4fb6.isResizeable()){
		...
		draw2d.Canvas.prototype.addFigure.call(this,this.resizeHandle2,xPos+(_4fba/2)-this.resizeHandleHalfWidth,yPos-_4fb8);
		draw2d.Canvas.prototype.addFigure.call(this,this.resizeHandle4,xPos+_4fba,yPos+(_4fb9/2)-(_4fb8/2));
		draw2d.Canvas.prototype.addFigure.call(this,this.resizeHandle6,xPos+(_4fba/2)-this.resizeHandleHalfWidth,yPos+_4fb9);
		draw2d.Canvas.prototype.addFigure.call(this,this.resizeHandle8,xPos-_4fb7,yPos+(_4fb9/2)-(_4fb8/2));
...
draw2d.Workflow.prototype.snapToHelper=function(_4fd2,pos){
	if(this.snapToGeometryHelper!=null){
		if(_4fd2 instanceof draw2d.ResizeHandle){
			...
			if((_4fd6&draw2d.SnapToHelper.EAST_WEST)&&!(_4fd7&draw2d.SnapToHelper.EAST_WEST)){
				this.showSnapToHelperLineVertical(_4fd5.x);
				}			
			else{
				this.hideSnapToHelperLineVertical();
				}			
			if((_4fd6&draw2d.SnapToHelper.NORTH_SOUTH)&&!(_4fd7&draw2d.SnapToHelper.NORTH_SOUTH)){
			...
			var _4fd8=new draw2d.Dimension(pos.x,pos.y,_4fd2.getWidth(),_4fd2.getHeight());
			var _4fd5=new draw2d.Dimension(pos.x,pos.y,_4fd2.getWidth(),_4fd2.getHeight());
			var _4fd6=draw2d.SnapToHelper.NSEW;
			var _4fd7=this.snapToGeometryHelper.snapRectangle(_4fd8,_4fd5);
			if((_4fd6&draw2d.SnapToHelper.WEST)&&!(_4fd7&draw2d.SnapToHelper.WEST)){
				this.showSnapToHelperLineVertical(_4fd5.x);
				}			
			else{
				if((_4fd6&draw2d.SnapToHelper.EAST)&&!(_4fd7&draw2d.SnapToHelper.EAST)){
					this.showSnapToHelperLineVertical(_4fd5.getX()+_4fd5.getWidth());
					}				
				else{
					this.hideSnapToHelperLineVertical();
					}				
				}			
			if((_4fd6&draw2d.SnapToHelper.NORTH)&&!(_4fd7&draw2d.SnapToHelper.NORTH)){
				this.showSnapToHelperLineHorizontal(_4fd5.y);
				}			
			else{
				if((_4fd6&draw2d.SnapToHelper.SOUTH)&&!(_4fd7&draw2d.SnapToHelper.SOUTH)){
					this.showSnapToHelperLineHorizontal(_4fd5.getY()+_4fd5.getHeight());
					}				
				else{
					this.hideSnapToHelperLineHorizontal();
					}				
...
draw2d.LineColorDialog.prototype.onOk=function(){
	...
	if(typeof this.figure.setColor=="function"){
		_60b3.getCommandStack().execute(new draw2d.CommandSetColor(this.figure,this.getSelectedColor()));
...
draw2d.BackgroundColorDialog.prototype.onOk=function(){
	...
	if(typeof this.figure.setBackgroundColor=="function"){
		_4fe6.getCommandStack().execute(new draw2d.CommandSetBackgroundColor(this.figure,this.getSelectedColor()));
...
draw2d.CommandStack.prototype.execute=function(_4ef3){
	...
	//alert('push into undostack ' + ((_4ef3.figure.getModel) ? _4ef3.figure.getModel() : _4ef3));
...
draw2d.CommandDelete.prototype.undo=function(){
	if(this.figure instanceof draw2d.CompartmentFigure){
		for(var i=0;
		i<this.compartmentDeleteCommands.getSize();
		i++){
		...
		if(li.addEventListener){
			...
			if(li.attachEvent){
				li.attachEvent("onclick",function(event){
					...
					event.srcElement.menuItem.execute(diffX+_4baf,diffY+_4bb0);
					}				
				);
...
draw2d.AbstractObjectModel.prototype.firePropertyChange=function(_5162,_5163,_5164){
...
draw2d.AbstractObjectModel.prototype.removePropertyChangeListener=function(_5169){
	if(_5169!=null){
...
XMLP.prototype._checkStructure=function(_5b4d){
	...
	if(XMLP._STATE_MISC==this.m_iState){
		if((XMLP._ELM_B==_5b4d)||(XMLP._ELM_E==_5b4d)||(XMLP._ELM_EMP==_5b4d)||(XMLP.EVT_DTD==_5b4d)){
			return this._setErr(XMLP.ERR_DOC_STRUCTURE);
			}		
		if((XMLP._TEXT==_5b4d)||(XMLP._ENTITY==_5b4d)){
			if(SAXStrings.indexOfNonWhitespace(this.getContent(),this.getContentBegin(),this.getContentEnd())!=-1){
		...
		if(_5b7e==XMLP._ELM_B){
			...
			if(_5b7e==XMLP._ELM_E){
				...
				if(_5b7e==XMLP._ELM_EMP){
					...
					if(_5b7e==XMLP._TEXT){
						this._fireEvent(SAXDriver.CHARS,_5b7d.getContent(),_5b7d.getContentBegin(),_5b7d.getContentEnd()-_5b7d.getContentBegin());
						}					
					else{
						if(_5b7e==XMLP._ENTITY){
							...
							if(_5b7e==XMLP._PI){
								this._fireEvent(SAXDriver.PI,_5b7d.getName(),_5b7d.getContent().substring(_5b7d.getContentBegin(),_5b7d.getContentEnd()));
								}							
							else{
								if(_5b7e==XMLP._CDATA){
									...
									if(_5b7e==XMLP._COMMENT){
										this._fireEvent(SAXDriver.CMNT,_5b7d.getContent(),_5b7d.getContentBegin(),_5b7d.getContentEnd()-_5b7d.getContentBegin());
										}									
									else{
										if(_5b7e==XMLP._DTD){
											}										
											...
											if(_5b7e==XMLP._ERROR){
												this._fireError(_5b7d.getContent());
												}											
											else{
												if(_5b7e==XMLP._NONE){
													return;
...
function trim(_5b9b,_5b9c,_5b9d){
	...
	var k=0;
	if(_5b9c==true){
		while((i<_5b9b.length)&&(whitespace.indexOf(_5b9b.charAt(i++))!=-1)){
			left++;
			}		
		}	
	if(_5b9d==true){
...
DOMImplementation.prototype.hasFeature=function DOMImplementation_hasFeature(_4c06,_4c07){
	var ret=false;
...
DOMImplementation.prototype.translateErrCode=function DOMImplementation_translateErrCode(code){
	var msg="";
	switch(code){
		case DOMException.INDEX_SIZE_ERR:msg="INDEX_SIZE_ERR: Index out of bounds";
		break;
		case DOMException.DOMSTRING_SIZE_ERR:msg="DOMSTRING_SIZE_ERR: The resulting string is too long to fit in a DOMString";
		break;
		case DOMException.HIERARCHY_REQUEST_ERR:msg="HIERARCHY_REQUEST_ERR: The Node can not be inserted at this location";
		break;
		case DOMException.WRONG_DOCUMENT_ERR:msg="WRONG_DOCUMENT_ERR: The source and the destination Documents are not the same";
		break;
		case DOMException.INVALID_CHARACTER_ERR:msg="INVALID_CHARACTER_ERR: The string contains an invalid character";
		break;
		case DOMException.NO_DATA_ALLOWED_ERR:msg="NO_DATA_ALLOWED_ERR: This Node / NodeList does not support data";
		break;
		case DOMException.NO_MODIFICATION_ALLOWED_ERR:msg="NO_MODIFICATION_ALLOWED_ERR: This object cannot be modified";
		break;
		case DOMException.NOT_FOUND_ERR:msg="NOT_FOUND_ERR: The item cannot be found";
		break;
		case DOMException.NOT_SUPPORTED_ERR:msg="NOT_SUPPORTED_ERR: This implementation does not support function";
		break;
		case DOMException.INUSE_ATTRIBUTE_ERR:msg="INUSE_ATTRIBUTE_ERR: The Attribute has already been assigned to another Element";
		break;
		case DOMException.INVALID_STATE_ERR:msg="INVALID_STATE_ERR: The object is no longer usable";
		break;
		case DOMException.SYNTAX_ERR:msg="SYNTAX_ERR: Syntax error";
		break;
		case DOMException.INVALID_MODIFICATION_ERR:msg="INVALID_MODIFICATION_ERR: Cannot change the type of the object";
		break;
		case DOMException.NAMESPACE_ERR:msg="NAMESPACE_ERR: The namespace declaration is incorrect";
		break;
		case DOMException.INVALID_ACCESS_ERR:msg="INVALID_ACCESS_ERR: The object does not support this function";
		break;
		default:msg="UNKNOWN: Unknown Exception Code ("+code+")";
		}	
	return msg;
	}
;
			...
			if(iEvt==XMLP._ELM_E){
				...
				if(iEvt==XMLP._ELM_EMP){
					...
					if(iEvt==XMLP._TEXT||iEvt==XMLP._ENTITY){
						var _4c18=p.getContent().substring(p.getContentBegin(),p.getContentEnd());
						...
						if(iEvt==XMLP._PI){
							...
							if(iEvt==XMLP._CDATA){
								_4c18=p.getContent().substring(p.getContentBegin(),p.getContentEnd());
								if(!this.preserveWhiteSpace){
									_4c18=trim(_4c18,true,true);
									_4c18.replace(/ +/g," ");
									}								
								if(_4c18.length>0){
									iNodeParent.appendChild(doc.createCDATASection(_4c18));
									}								
								}							
							else{
								if(iEvt==XMLP._COMMENT){
									...
									if(_4c18.length>0){
										iNodeParent.appendChild(doc.createComment(_4c18));
										}									
									}								
								else{
									if(iEvt==XMLP._DTD){
...
DOMImplementation.prototype._isNamespaceDeclaration=function DOMImplementation__isNamespaceDeclaration(_4c23){
	return (_4c23.indexOf("xmlns")>-1);
	}
;

DOMImplementation.prototype._isIdDeclaration=function DOMImplementation__isIdDeclaration(_4c24){
...
DOMImplementation.prototype._parseNSName=function DOMImplementation__parseNSName(_4c27){
...
DOMImplementation.prototype._parseQName=function DOMImplementation__parseQName(_4c29){
...
DOMNodeList.prototype._insertBefore=function DOMNodeList__insertBefore(_4c32,_4c33){
...
DOMNodeList.prototype._replaceChild=function DOMNodeList__replaceChild(_4c35,_4c36){
	var ret=null;
...
DOMNodeList.prototype._cloneNodes=function DOMNodeList__cloneNodes(deep,_4c3e){
...
DOMNamedNodeMap.prototype.getNamedItemNS=function DOMNamedNodeMap_getNamedItemNS(_4c4f,_4c50){
	var ret=null;
...
DOMNamedNodeMap.prototype.removeNamedItemNS=function DOMNamedNodeMap_removeNamedItemNS(_4c56,_4c57){
	var ret=null;
	if(this.ownerDocument.implementation.errorChecking&&(this._readonly||(this.parentNode&&this.parentNode._readonly))){
		throw (new DOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR));
		}	
...
DOMNamedNodeMap.prototype._findNamedItemNSIndex=function DOMNamedNodeMap__findNamedItemNSIndex(_4c5e,_4c5f){
	var ret=-1;
	if(_4c5f){
		for(var i=0;
		i<this._nodes.length;
		i++){
			if((this._nodes[i].namespaceURI==_4c5e)&&(this._nodes[i].localName==_4c5f)){
				ret=i;
				break;
				}			
			}		
...
DOMNamedNodeMap.prototype._hasAttributeNS=function DOMNamedNodeMap__hasAttributeNS(_4c65,_4c66){
	var ret=false;
...
DOMNamedNodeMap.prototype._cloneNodes=function DOMNamedNodeMap__cloneNodes(_4c69){
...
DOMNamespaceNodeMap.prototype._findNamedItemIndex=function DOMNamespaceNodeMap__findNamedItemIndex(_4c70){
	var ret=-1;
	for(var i=0;
	i<this._nodes.length;
	i++){
...
DOMNamespaceNodeMap.prototype._cloneNodes=function DOMNamespaceNodeMap__cloneNodes(_4c73){
...
DOMNode.prototype.setPrefix=function DOMNode_setPrefix(_4c7b){
	if(this.ownerDocument.implementation.errorChecking){
		...
		if(!this.ownerDocument._isValidNamespace(this.namespaceURI,_4c7b+":"+this.localName)){
			throw (new DOMException(DOMException.NAMESPACE_ERR));
			}		
		if((_4c7b=="xmlns")&&(this.namespaceURI!="http://www.w3.org/2000/xmlns/")){
			throw (new DOMException(DOMException.NAMESPACE_ERR));
			}		
...
DOMNode.prototype.insertBefore=function DOMNode_insertBefore(_4c7c,_4c7d){
	...
	if(_4c7d){
		...
		this.childNodes._insertBefore(_4c7c,this.childNodes._findItemIndex(_4c7d._id));
		_4c7e=_4c7d.previousSibling;
		if(_4c7c.nodeType==DOMNode.DOCUMENT_FRAGMENT_NODE){
			if(_4c7c.childNodes._nodes.length>0){
				for(var ind=0;
				ind<_4c7c.childNodes._nodes.length;
				ind++){
					_4c7c.childNodes._nodes[ind].parentNode=this;
					}				
				_4c7d.previousSibling=_4c7c.childNodes._nodes[_4c7c.childNodes._nodes.length-1];
				}			
			}		
		else{
...
DOMNode.prototype.replaceChild=function DOMNode_replaceChild(_4c82,_4c83){
	...
	if(_4c82.nodeType==DOMNode.DOCUMENT_FRAGMENT_NODE){
		if(_4c82.childNodes._nodes.length>0){
			for(var ind=0;
			ind<_4c82.childNodes._nodes.length;
			ind++){
			...
			if(_4c83.nextSibling){
				...
				this.lastChild=_4c82.childNodes._nodes[_4c82.childNodes._nodes.length-1];
				}			
...
DOMNode.prototype.removeChild=function DOMNode_removeChild(_4c88){
	if(this.ownerDocument.implementation.errorChecking&&(this._readonly||_4c88._readonly)){
		throw (new DOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR));
		}	
...
DOMNode.prototype.appendChild=function DOMNode_appendChild(_4c8a){
	...
	if(_4c8a.nodeType==DOMNode.DOCUMENT_FRAGMENT_NODE){
		if(_4c8a.childNodes._nodes.length>0){
			for(var ind=0;
			ind<_4c8a.childNodes._nodes.length;
			ind++){
			...
			if(this.lastChild){
				...
				this.lastChild=_4c8a.childNodes._nodes[_4c8a.childNodes._nodes.length-1];
				}			
			else{
				this.lastChild=_4c8a.childNodes._nodes[_4c8a.childNodes._nodes.length-1];
...
DOMNode.prototype.getElementsByTagName=function DOMNode_getElementsByTagName(_4c94){
	return this._getElementsByTagNameRecursive(_4c94,new DOMNodeList(this.ownerDocument));
	}
;

DOMNode.prototype._getElementsByTagNameRecursive=function DOMNode__getElementsByTagNameRecursive(_4c95,_4c96){
	if(this.nodeType==DOMNode.ELEMENT_NODE||this.nodeType==DOMNode.DOCUMENT_NODE){
		...
		i<this.childNodes.length;
		i++){
			_4c96=this.childNodes.item(i)._getElementsByTagNameRecursive(_4c95,_4c96);
			}		
		}	
	return _4c96;
	}
...
DOMNode.prototype.getElementsByTagNameNS=function DOMNode_getElementsByTagNameNS(_4c98,_4c99){
	return this._getElementsByTagNameNSRecursive(_4c98,_4c99,new DOMNodeList(this.ownerDocument));
	}
;

DOMNode.prototype._getElementsByTagNameNSRecursive=function DOMNode__getElementsByTagNameNSRecursive(_4c9a,_4c9b,_4c9c){
	if(this.nodeType==DOMNode.ELEMENT_NODE||this.nodeType==DOMNode.DOCUMENT_NODE){
		if(((this.namespaceURI==_4c9a)||(_4c9a=="*"))&&((this.localName==_4c9b)||(_4c9b=="*"))){
			_4c9c._appendChild(this);
			}		
		for(var i=0;
		i<this.childNodes.length;
		i++){
			_4c9c=this.childNodes.item(i)._getElementsByTagNameNSRecursive(_4c9a,_4c9b,_4c9c);
			}		
		}	
	return _4c9c;
	}
	...
	try{
		if(_4c9f.nodeType==DOMNode.ELEMENT_NODE){
			if(!this.ownerDocument.implementation.namespaceAware){
				...
				for(var i=0;
				i<_4c9f.attributes.length;
				i++){
					_4ca1.setAttribute(_4c9f.attributes.item(i).name,_4c9f.attributes.item(i).value);
					}				
				}			
			else{
				_4ca1=this.ownerDocument.createElementNS(_4c9f.namespaceURI,_4c9f.nodeName);
				for(var i=0;
				i<_4c9f.attributes.length;
				i++){
					_4ca1.setAttributeNS(_4c9f.attributes.item(i).namespaceURI,_4c9f.attributes.item(i).name,_4c9f.attributes.item(i).value);
					}				
				for(var i=0;
				i<_4c9f._namespaces.length;
				i++){
					_4ca1._namespaces._nodes[i]=this.ownerDocument.createNamespace(_4c9f._namespaces.item(i).localName);
					_4ca1._namespaces._nodes[i].setValue(_4c9f._namespaces.item(i).value);
					}				
				}			
			}		
		else{
			if(_4c9f.nodeType==DOMNode.ATTRIBUTE_NODE){
				if(!this.ownerDocument.implementation.namespaceAware){
					...
					_4ca1=this.ownerDocument.createAttributeNS(_4c9f.namespaceURI,_4c9f.nodeName);
					for(var i=0;
					i<_4c9f._namespaces.length;
					i++){
						_4ca1._namespaces._nodes[i]=this.ownerDocument.createNamespace(_4c9f._namespaces.item(i).localName);
				...
				if(_4c9f.nodeType==DOMNode.DOCUMENT_FRAGMENT){
					...
					if(_4c9f.nodeType==DOMNode.NAMESPACE_NODE){
						_4ca1=this.ownerDocument.createNamespace(_4c9f.nodeName);
						_4ca1.setValue(_4c9f.value);
						}					
					else{
						if(_4c9f.nodeType==DOMNode.TEXT_NODE){
							_4ca1=this.ownerDocument.createTextNode(_4c9f.data);
							}						
						else{
							if(_4c9f.nodeType==DOMNode.CDATA_SECTION_NODE){
								_4ca1=this.ownerDocument.createCDATASection(_4c9f.data);
								}							
							else{
								if(_4c9f.nodeType==DOMNode.PROCESSING_INSTRUCTION_NODE){
									_4ca1=this.ownerDocument.createProcessingInstruction(_4c9f.target,_4c9f.data);
									}								
								else{
									if(_4c9f.nodeType==DOMNode.COMMENT_NODE){
										_4ca1=this.ownerDocument.createComment(_4c9f.data);
										}									
									else{
		...
		if(deep){
			for(var i=0;
			i<_4c9f.childNodes.length;
			i++){
				_4ca1.appendChild(this.ownerDocument.importNode(_4c9f.childNodes.item(i),true));
				}			
			}		
...
DOMDocument.prototype.createElement=function DOMDocument_createElement(_4ca6){
	if(this.ownerDocument.implementation.errorChecking&&(!this.ownerDocument.implementation._isValidName(_4ca6))){
		throw (new DOMException(DOMException.INVALID_CHARACTER_ERR));
		}	
...
DOMDocument.prototype.createProcessingInstruction=function DOMDocument_createProcessingInstruction(_4caf,data){
	if(this.ownerDocument.implementation.errorChecking&&(!this.implementation._isValidName(_4caf))){
		throw (new DOMException(DOMException.INVALID_CHARACTER_ERR));
		}	
...
DOMDocument.prototype.createElementNS=function DOMDocument_createElementNS(_4cb4,_4cb5){
	if(this.ownerDocument.implementation.errorChecking){
...
DOMDocument.prototype.createAttributeNS=function DOMDocument_createAttributeNS(_4cb8,_4cb9){
	if(this.ownerDocument.implementation.errorChecking){
...
DOMDocument.prototype.createNamespace=function DOMDocument_createNamespace(_4cbc){
...
DOMDocument.prototype.getElementById=function DOMDocument_getElementById(_4cbf){
	retNode=null;
	for(var i=0;
	i<this.all.length;
	i++){
		var node=this.all[i];
		if((node.id==_4cbf)&&(node._isAncestor(node.ownerDocument.documentElement))){
			retNode=node;
			break;
			}		
		}	
...
DOMDocument.prototype._isValidNamespace=function DOMDocument__isValidNamespace(_4cc2,_4cc3,_4cc4){
	...
	if((valid)&&(qName.prefix=="xml")&&(_4cc2!="http://www.w3.org/XML/1998/namespace")){
		valid=false;
		}	
	return valid;
	}
...
DOMElement.prototype.setAttributeNode=function DOMElement_setAttributeNode(_4cd0){
...
DOMElement.prototype.removeAttributeNode=function DOMElement_removeAttributeNode(_4cd1){
...
DOMElement.prototype.getAttributeNS=function DOMElement_getAttributeNS(_4cd3,_4cd4){
	var ret="";
...
DOMElement.prototype.setAttributeNS=function DOMElement_setAttributeNS(_4cd7,_4cd8,value){
...
DOMElement.prototype.removeAttributeNS=function DOMElement_removeAttributeNS(_4cdb,_4cdc){
	return this.attributes.removeNamedItemNS(_4cdb,_4cdc);
	}
;

DOMElement.prototype.getAttributeNodeNS=function DOMElement_getAttributeNodeNS(_4cdd,_4cde){
	return this.attributes.getNamedItemNS(_4cdd,_4cde);
	}
;

DOMElement.prototype.setAttributeNodeNS=function DOMElement_setAttributeNodeNS(_4cdf){
	if((_4cdf.prefix=="")&&this.ownerDocument.implementation._isIdDeclaration(_4cdf.name)){
		this.id=_4cdf.value;
		}	
...
DOMElement.prototype.hasAttributeNS=function DOMElement_hasAttributeNS(_4ce1,_4ce2){
...
DOMCharacterData.prototype.substringData=function DOMCharacterData_substringData(_4cf0,count){
	var ret=null;
	if(this.data){
		if(this.ownerDocument.implementation.errorChecking&&((_4cf0<0)||(_4cf0>this.data.length)||(count<0))){
...
DOMCharacterData.prototype.insertData=function DOMCharacterData_insertData(_4cf4,arg){
	if(this.ownerDocument.implementation.errorChecking&&this._readonly){
		throw (new DOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR));
		}	
	if(this.data){
		if(this.ownerDocument.implementation.errorChecking&&((_4cf4<0)||(_4cf4>this.data.length))){
			throw (new DOMException(DOMException.INDEX_SIZE_ERR));
			}		
		this.setData(this.data.substring(0,_4cf4).concat(arg,this.data.substring(_4cf4)));
		}	
	else{
...
DOMCharacterData.prototype.deleteData=function DOMCharacterData_deleteData(_4cf6,count){
	if(this.ownerDocument.implementation.errorChecking&&this._readonly){
		throw (new DOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR));
		}	
	if(this.data){
		if(this.ownerDocument.implementation.errorChecking&&((_4cf6<0)||(_4cf6>this.data.length)||(count<0))){
			throw (new DOMException(DOMException.INDEX_SIZE_ERR));
			}		
		if(!count||(_4cf6+count)>this.data.length){
			...
			this.setData(this.data.substring(0,_4cf6).concat(this.data.substring(_4cf6+count)));
			}		
		}	
	}
;

DOMCharacterData.prototype.replaceData=function DOMCharacterData_replaceData(_4cf8,count,arg){
	if(this.ownerDocument.implementation.errorChecking&&this._readonly){
		throw (new DOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR));
		}	
	if(this.data){
		if(this.ownerDocument.implementation.errorChecking&&((_4cf8<0)||(_4cf8>this.data.length)||(count<0))){
			throw (new DOMException(DOMException.INDEX_SIZE_ERR));
			}		
		this.setData(this.data.substring(0,_4cf8).concat(arg,this.data.substring(_4cf8+count)));
		}	
	else{
		this.setData(arg);
		}	
...
draw2d.XMLSerializer.toXML=function(obj,_48e3,_48e4){
	...
	switch(t){
		case "int":case "number":case "string":case "boolean":s+=obj;
		break;
		case "date":s+=obj.toLocaleString();
		break;
		case "Array":case "array":s+="\n";
		for(var i=0;
		i<obj.length;
		i++){
...
draw2d.XMLSerializer.isSimpleVar=function(t){
	switch(t){
		case "int":case "string":case "String":case "Number":case "number":case "Boolean":case "boolean":case "bool":case "dateTime":case "Date":case "date":case "float":return true;
		}	
	return false;
	}
;
...
draw2d.XMLDeserializer.fromXML=function(node,_4d75){
	...
	switch(_4d76){
		case "int":return parseInt(""+node.getChildNodes().item(0).getNodeValue());
		case "string":case "String":return ""+node.getChildNodes().item(0).getNodeValue();
		case "Number":case "number":return parseFloat(""+node.getChildNodes().item(0).getNodeValue());
		case "Boolean":case "boolean":case "bool":return parseBool(""+node.getChildNodes().item(0).getNodeValue());
		case "dateTime":case "Date":case "date":return new Date(""+node.getChildNodes().item(0).getNodeValue());
		case "float":return parseFloat(""+node.getChildNodes().item(0).getNodeValue());
		break;
		}	
	...
	i++){
		...
		obj[_4d7c]=draw2d.XMLDeserializer.fromXML(child,obj instanceof draw2d.AbstractObjectModel?obj:_4d75);
		}	
	return obj;
	}
;
