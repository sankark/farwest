...
template_to_html(Req, State=#state{path=Paths}) ->
	...
	{ok, Data} = fw_templates_server:get_template(T2),
	case Parts of
		false ->
	{<<"value">>, Contents} = lists:keyfind(<<"value">>, 1, Data),
					Req2 = cowboy_req:set_resp_header(<<"content-type">>,mime_type(classify_extension(".dtl")),Req),
					  send_page(Contents,Req2,State);
	 <<"meta">> -> Meta = get_file_meta(Template),
						          Body = jsx:encode(Meta),
...
template_from_form(Req, State=#state{path=Paths}) ->
	...
	[Template] = Paths,
	T2 = filename:basename(binary_to_list(Template),".dtl"),
	fw_templates_server:set_template(list_to_binary(T2), <<"admin">>, Body, <<"admin">>),
	Meta = get_file_meta(Template),
	Resp_Body = jsx:encode(Meta),
