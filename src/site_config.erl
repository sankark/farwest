...
-record(state, {config_file,config=[]}).
...
init(Args) ->
   		SiteHome = util:get_site_home(Args),
		ConfigFile = filename:join(SiteHome,"config"),
		{ok,Config}= file:consult(ConfigFile),
        {ok, #state{config_file = ConfigFile,config=hd(Config)}}.
...
handle_call({get_config}, _From, State=#state{config_file=File}) ->
   {reply,File,State};
...
config_file() ->
    filename:join([code:priv_dir(farwest),"config"]).
