...
handle_call({get_keys, Bucket}, _, State=#state{riak_pid=RiakPid}) ->
	{reply, do_get_keys(RiakPid, Bucket), State};
...
do_get_all_values(RiakPid, Bucket)->
...
do_get_keys(RiakPid, Bucket)->
	fw_versioned_buckets:keys(RiakPid, Bucket).

do_get_data(RiakPid, Bucket , Key)->
			fw_versioned_buckets:get(RiakPid, Bucket, Key).

do_get_value(RiakPid, Bucket , Key)->
...
do_set_data(RiakPid, Bucket, Key,EditBy, UserData, Comments) ->
